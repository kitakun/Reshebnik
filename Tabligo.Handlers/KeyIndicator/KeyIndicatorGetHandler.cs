...
namespace Tabligo.Handlers.KeyIndicator;
        ...
        CancellationToken ct = default)
    {
        ...
        foreach (var group in indicators.GroupBy(i => i.Category).Where(g => g.Any()))
        {
            ...
            foreach (var ind in group)
            {
                ...
                var needExpand = ComparePeriods(metricPeriod, type) > 0;
                var expected = needExpand ? metricPeriod : type;
                var last12Range = type == PeriodTypeEnum.Custom ? range : BuildRangeForPeriod(range.To, expected);
                ...
                if (needExpand)
                {
                    plan = ExpandTo(plan, last12Range.From, range.To, expected, type);
                    fact = ExpandTo(fact, last12Range.From, range.To, expected, type);
                }
                else if (type != PeriodTypeEnum.Custom)
                {
